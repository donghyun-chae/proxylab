...
int main(int argc, char *argv[]) {
  ...
  listenfd = Open_listenfd(argv[1]);  // 듣기 소켓 열기
  while (1) {
...
void doit(int fd) {
  ...
  if (is_static) { /* Serve static content */
    ...
    serve_static(fd, filename, sbuf.st_size, head_r_flag);
  } else { /* Serve dynamic content */
    if(!(S_ISREG(sbuf.st_mode)) || !(S_IXUSR & sbuf.st_mode)) {
      clienterror(fd, filename, "403", "Forbidden", "Tiny couldn't run the CGI program");
      return;
    }
    serve_dynamic(fd, filename, cgiargs, head_r_flag);
  }
}
...
void serve_static(int fd, char* filename, int filesize, int head_r_flag) {
  int srcfd;
  char* srcp;
  ...
  bytes_read = Rio_readn(srcfd, srcp, filesize);
  if (bytes_read != filesize) {
    fprintf(stderr, "Error: Read %zd bytes, expected %d bytes\n", bytes_read, filesize);
    Free(srcp);
    Close(srcfd);
    return;
  }
